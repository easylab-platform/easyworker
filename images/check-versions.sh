...
p NODE "26.8.2" "$(curl -s https://nodejs.org/dist/index.json | json 'print("current",d[0]["version"],"| LTS",[x["version"] for x in d if x.get("lts")][0])')"
p GO "1.27.1" "$(curl -s 'https://go.dev/dl/?mode=json' | json 'print([x["version"] for x in d if x["stable"]][0])')"
p UV "0.12.14" "$(ghl astral-sh/uv)"
...
p JDK "25.0.4.1+1" "$(ghl adoptium/temurin25-binaries) [LTS; 26 is latest feature, 27 not on Adoptium]"
...
p OTP "29.0.6" "$(curl -s https://builds.hex.pm/builds/otp/ubuntu-22.04/builds.txt | awk '{print $1}' | grep -E '^OTP-[0-9]+\.[0-9]+(\.[0-9]+)?$' | sort -V | tail -1)"
...
p CLOJURE "1.12.0.1530" "$(ghl clojure/brew-install)"
p SCALA_CLI "1.17.0" "$(ghl VirtusLab/scala-cli)"
p SBT "1.11.6" "$(gh api 'repos/sbt/sbt/releases?per_page=50' --jq '[.[]|select(.tag_name|startswith("v1."))][0].tag_name' 2>/dev/null || echo '(?)')"
...
p OPAM "2.5.2" "$(ghl ocaml/opam)"
p GHCUP "0.2.6.2" "$(curl -s https://downloads.haskell.org/~ghcup/ | grep -oE '0\.2\.[0-9]+\.[0-9]+' | sort -V | tail -1)"
p ZIG "0.15.2" "$(curl -s https://ziglang.org/download/index.json | json 'print([k for k in d if k!="master"][0])')"
p CPANM "1.7047" "$(curl -s https://fastapi.metacpan.org/v1/release/App-cpanminus | json 'print(d.get("version"))')"
p LUA "5.4.7" "$(curl -s https://www.lua.org/ftp/ | grep -oE 'lua-5\.[0-9]+\.[0-9]+\.tar\.gz' | sort -V | tail -1)"
p LUAROCKS "3.13.0" "$(ghl luarocks/luarocks)"
p R "4.5.1" "$(curl -s https://cran.r-project.org/src/base/R-4/ | grep -oE 'R-4\.[0-9]+\.[0-9]+\.tar\.gz' | sort -V | tail -1)"
